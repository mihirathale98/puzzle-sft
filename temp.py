    ...
    while len(problems) < target_count:
        ...
        if problem["prompt"] not in problem_prompts:
            ...
            if len(problems) % 100 == 0:
                print(f"Generated {len(problems)} unique problems so far...")
    
    return problems

# ------------------- ASYNC API INTERACTION -------------------

async def query_model_async(client, prompt, semaphore, max_retries=3):
    """
    Asynchronously query the model with retries in case of connection errors
    """
    for attempt in range(max_retries):
        try:
            async with semaphore:  # Control concurrency with semaphore
                # Enhanced prompt to encourage step-by-step reasoning
                full_prompt = prompt
                
                # Use AsyncOpenAI client for async requests
...
# ------------------- PROCESS PROBLEM BATCH -------------------

async def process_problem_batch(problems: List[Dict], client, dataset: List[Dict], 
                               semaphore, output_file: str, batch_size: int = 32):
    """
    Process a batch of problems asynchronously
    """
    ...
    for i in range(0, len(problems), batch_size):
        ...
        print(f"Processing batch {i//batch_size + 1}/{(len(problems)-1)//batch_size + 1}")
        
        # Track the tasks
        tasks = []
        for j, problem in enumerate(batch):
            problem_idx = i + j
            print(f"Queuing problem {problem_idx+1}/{len(problems)}: {problem['prompt']}")
            
            # Update template counter
            template_counts[problem['template']] += 1
            
            # Create task for processing this problem
            task = asyncio.create_task(
                process_single_problem(problem, problem_idx, client, 
                                      semaphore, dataset, output_file)
            )
            tasks.append(task)
        ...
        # Display distribution after each batch
        print("\nCurrent template distribution:")
        for template, count in template_counts.items():
            print(f"  {template}: {count} ({count/sum(template_counts.values())*100:.1f}%)")
        print()
    ...
    print(f"\nCompleted with {len(dataset)} total samples")

async def process_single_problem(problem, problem_idx, client, semaphore, dataset, output_file):
    """
    Process a single probability problem
    """
    print(f"Processing problem {problem_idx+1}: {problem['prompt']}")
    print(f"Template: {problem['template']}")
    
    # Try up to 3 attempts for each problem
    for attempt in range(3):
        print(f"Attempt {attempt+1}...")
        
        # Query the model asynchronously
        response = await query_model_async(client, problem['prompt'], semaphore)
        
        # Check if the answer is correct
        is_correct = is_answer_correct(response['content'], problem['correct_answer'])
        ...
        # If correct or last attempt, save and break
        if is_correct or attempt == 2:
            dataset.append(sample)
            print(f"Saved sample {len(dataset)} (Correct: {is_correct})")
            break
...
async def main_async(target_count=1000, output_file="probability_dataset.json", batch_size=32, concurrency_limit=32):
    """
    Async main function to generate, solve, and validate problems in parallel
    """
    # Initialize AsyncOpenAI client
    client = AsyncOpenAI(
        base_url="https://48be-149-7-4-5.ngrok-free.app/v1", 
        api_key="token-abc-sundai"
    )
    ...
    # Randomize the order of problems to ensure diverse processing
    ...
    await process_problem_batch(problems, client, dataset, semaphore, output_file, batch_size)

# ------------------- ENTRY POINT -------------------

def main(target_count=1000, output_file="probability_dataset.json", batch_size=32, concurrency_limit=32):
    """
    Main function that runs the async event loop
    """
    asyncio.run(main_async(target_count, output_file, batch_size, concurrency_limit))

if __name__ == "__main__":
    main(target_count=2000, output_file="probability_dataset_2.json", batch_size=32, concurrency_limit=32)